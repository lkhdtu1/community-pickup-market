...
- **MVC (Model-View-Controller)**: Used to separate business logic, data, and user interface. The backend is structured with models representing data, views handling the presentation, and controllers managing the business logic.
- **Repository Pattern**: Implemented for data access, providing a clean separation between the data access logic and the business logic. This pattern is used in the backend to abstract database operations, making it easier to switch between different data sources.
- **Factory Pattern**: Used for creating objects without specifying the exact class of object that will be created. This pattern is applied in the frontend to instantiate UI components dynamically based on user interactions.
- **Singleton Pattern**: Applied to ensure a class has only one instance, such as database connections. This pattern is used in the backend to manage shared resources efficiently, like database connections and configuration settings.
